...
function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const metricsRes = await axios.get(`${API_URL}/metrics/${res.data.version._id}`);
      setData({ ...res.data, metrics: metricsRes.data });
      setError(null);
    } catch (err) {
      console.error(err);
  ...
  const handleReplan = async (disruption: any) => {
    setLoading(true);
    try {
      const latestRes = await axios.get(`${API_URL}/schedule/latest`);
      const versionId = latestRes.data.version._id;
      
      const replanRes = await axios.post(`${API_URL}/schedule/replan/${versionId}`, disruption);
      const newVersionId = replanRes.data._id;
      
      const diffRes = await axios.get(`${API_URL}/diff/${newVersionId}`);
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <header className="flex justify-between items-center bg-white p-6 rounded-lg shadow-sm">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Placement Week Coordinator Dashboard</h1>
            <p className="text-gray-500">Live Scheduling and Dynamic Replanning System</p>
          </div>
          <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            <SimulationPanel onReplan={handleReplan} loading={loading} />
          </div>
        </div>
